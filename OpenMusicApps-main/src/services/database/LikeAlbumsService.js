...
class LikeAlbumsService {
  ...
  async likeAlbums(userId, albumId) {
    ...
    if (result.rows.length) {
      throw new VarianError('Hanya bisa Like Album Sekali');
    }
  ...
  async unlikeAlbums(userId, albumId) {
  ...
  async getLikesCount(albumId) {
    try {
      ...
      return [parseInt(JSON.parse(result).count), true];
    } catch (error) {
      const query = {
      ...
      return [parseInt(result.rows[0].count), false];
    }
  }
}
